...
import { getWorkspaceDir, getWorkspaceDirs, FilePickerSearchtDatabaseFile, log, logv, ensureCacheDirSync } from "./constants";
...
export async function loadSearchDatabaseAsync() {
    ...
    // once shot
    await Promise.all(getWorkspaceDirs().map(workspaceFolder => {
        return new Promise((resolve, reject) => {
            ...
            log("filepicker: loading search datababse from " + cacheFile)
            ...
            readInterface.on('close', () => {
                log("filepicker: search datababse loaded");
...
export async function addExcludeDirs(dirs: string[]) {
    ...
    var newExcludeDirs = dirs.sort((a, b) => a.length - b.length).filter((dir: string) => {
        ...
        if (!dir.startsWith(workspaceDir)) {
            log("filepicker: #updateIncludeAndExcludeDirs, illegal dir: " + dir);
            return false;
        }
...
function loadExcludedDirs() {
    ...
    var excludeDirs = new Set<String>(excludeDirsConfig.split(":").concat(default_exclude_dirs).filter((elem: string) => {
...
export async function buildSearchDatabase() {
    ...
    for(var workspaceFolder of getWorkspaceDirs()) {
        ...
        myStatusBarItem.hide();        
        stream.end();
        log("filepicker: found " + file_list.length + " files");
